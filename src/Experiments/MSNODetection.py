...
class MSNODetectionRAM(ODRAM):

    def __init__(self, config=Config()):
        super(MSNODetectionRAM, self).__init__(config=config)
...
def main():
    ...
    ramConfig = Config(isTrain=True,
                       ...
                       maxTrainEpoch=40,
                       keepProb=0.9,
                       modelRelativeDir='output/RAM/MSNO_D_RAM')

    # Create ram object
    ram = MSNODetectionRAM(config=ramConfig)
