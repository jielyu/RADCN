...
class MSOClasRAM(ClaRAM):
    
    def __init__(self, config=Config()):
        super(MSOClasRAM, self).__init__(config=config)
...
def main():
    ...
    config = Config(isTrain=isTrain,
                    ...
                    maxTrainEpoch=100,
                    keepProb=0.9,
                    modelRelativeDir='output/RAM/MSO_C_RAM')
    # Create ram object
    ram = MSOClasRAM(config=config)
