...
class Config(SuperConfig):
    ...
    def __init__(self,
        ...
        # Conv1
        self.w1Shape = [5, 5, self.inputShape[2], 20]
        self.b1Shape = [self.w1Shape[3]]
        self.pool1Shape = [1, 2, 2, 1]
        # Conv2
        self.w2Shape = [5, 5, self.w1Shape[3], 50]
            ...
            if i == 0 \
                ...
                if i == 0 \
                    ...
                    Utils.saveModel(sess=sess,
                                    ...
                                    modelFileName=self.config.modelFileName)
                    # Test on testset
                    teLoss, teOverlap= self.testCDModel(sess=sess)
    ...
    def evaluate(self,
        ...
        for j in range(0, testSampleNum):
            ...
            feed_dict = {self.images: t_images,
                         self.objects: t_bbox}
            # Run graph
            loss, overlap, msr, objList = \
                sess.run((self.loss, self.overlap, self.msr, self.objList),
                         feed_dict=feed_dict)
